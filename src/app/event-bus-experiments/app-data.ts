import * as _ from "lodash";
import { Lesson } from '../shared/model/lesson';


export interface Observer {
  next(data:any);
}

export interface Observable {
  subscribe(obs:Observer);
  unsubscribe(obs:Observer);
}

// we don't want to leak Subject to the rest of the application: we keep it private
interface Subject extends Observer, Observable{

}

class SubjectImplementation implements Subject{
  private observers: Observer [] = []
  next(data:any){
    this.observers.forEach(obs => obs.next(data));
  }

  subscribe(obs:Observer){
    this.observers.push(obs)
  }

  unsubscribe(obs: Observer){
    _.remove(this.observers, el => el === obs)
  }
}


class DataStore implements Observable {
  private lessons : Lesson [] = []
  private lessonsListSubject = new SubjectImplementation();


  subscribe(obs: Observer){
      this.lessonsListSubject.subscribe(obs);
      obs.next(this.lessons); // any late subscriber (after the ngInit been called on our list, we still want to retrieve the lessons  )
  }

  unsubscribe(obs: Observer){
    this.lessonsListSubject.unsubscribe(obs)
    obs.next(this.lessons); // any late subscriber (after the ngInit been called on our list, we still want to retrieve the lessons  )
  }

  public initializeLessonsList(newList : Lesson[]){
    this.lessons = _.cloneDeep(newList);
    this.broadcast()
  }

  public addLesson(newLesson: Lesson) {
    this.lessons.push(_.cloneDeep(newLesson));
    this.broadcast()
  }

  public broadcast(){
    this.lessonsListSubject.next(_.cloneDeep(this.lessons));
  }

  public toggleLessonViewed(toggled:Lesson){
    const lesson = _.find(this.lessons, lesson => lesson.id === toggled.id)
    lesson.completed = !lesson.completed;
    this.broadcast()
  }

  public deleteLesson(deleted:Lesson){
    _.remove(this.lessons, lesson => lesson.id === deleted.id)
    this.broadcast();
  }
}


export const store = new DataStore();
